...
<body>
    ...
    <h1>Marcar Revisado Instalaciones</h1>
...
<table class="instalaciones-table">
    ...
    <tr>
        ...
        <td><%= espacio %></td>
        <td><%= pendiente %></td>
        <td><input type="checkbox" name="miCheckbox" value="<%= id %>"></td>
    </tr>
    <%
    }
    %>
...
<form id="myFormRevisado" action="back.jsp" method="post">
    <!-- Campo de entrada oculto para enviar la cadena generada -->
    <input type="hidden" id="idsMarcadosInput" name="idsMarcadosInputRevisar">
    <input type="hidden" id="codigo" name="codigo" value="2">
...
<script>
    ...
    function aplicarFiltro() {
          ...
          for (let i = 1; i < filas.length; i++) {
                ...
                const rev = fila.querySelector('td:nth-child(5)').textContent.toLowerCase();
                // Comprobar si el nombre del país contiene el filtro y el ID coincide
                if (pais.includes(filtroP) && ciudad.includes(filtroC) && instalacion.includes(filtroI) && (!isNaN(filtroNumID) ? id === filtroNumID : true) && rev == "false" ) {
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    checkboxes.forEach(function(checkbox) {
        checkbox.addEventListener("change", function() {
            ...
            espacioDiv.textContent = "Tamaño: " + resultado;
        });
    });
});
</script>
